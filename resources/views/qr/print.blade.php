...
<body>
  ...
  <div class="header">
    <h2>Kartu QR Code {{ $type === 'siswa' ? 'Siswa' : 'Guru' }}</h2>
    <p>{{ $groupInfo }}</p>
  </div>
  ...
  <div class="qr-grid">
    @foreach ($items as $item)
      <div class="qr-card">
        <img src="{{ $item['qr_url'] }}" alt="QR {{ $item['nama'] }}">
        <div class="name">{{ $item['nama'] }}</div>
        <div class="nip">{{ $item['nomor_label'] }}: {{ $item['nomor'] }}</div>
        @if (!empty($item['kelas']))
          <div class="kelas-label">{{ $item['kelas'] }}</div>
        @endif
      </div>
    @endforeach
  </div>
