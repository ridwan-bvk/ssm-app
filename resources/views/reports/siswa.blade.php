...
<body>
    ...
    <p>Bulan: {{ $bulan->translatedFormat('F Y') }} &mdash; Kelas: {{ $kelas->tingkat }} {{ $kelas->jurusan?->jurusan }} {{ $kelas->index_kelas }}</p>
    ...
    <table class="matrix">
        ...
        @foreach ($siswa as $index => $s)
            @php
                $row = $cells[$s->id_siswa] ?? [];
                $counts = ['h' => 0, 's' => 0, 'i' => 0, 'a' => 0];
                foreach ($row as $date => $idKehadiran) {
                    $isFuture = \Illuminate\Support\Carbon::parse($date)->isFuture();
                    if ($isFuture) {
                        continue;
                    }
                    match ($idKehadiran) {
    ...
    <table class="info">
        <tr><td>Jumlah siswa</td><td>: {{ $siswa->count() }}</td></tr>
